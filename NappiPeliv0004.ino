...
void Menu()                           //Menu itself
{
  ...
  if(val1 == 1)                         //Check if you press 1st button and turn on the game menu if you do.
  {
    gameMenu = true;
    delay(500);
  }
...
void GameMenu()
{
  ...
  if(val1 == 1)                         //Check which button has bee pressed and turn on the menu needed for that one
  {
    game1 = true;
    game2 = false;
    game3 = false;
  ...
  if(val4 == 1)                         //4th button will start the game
  {
...
void ReactionGame(){  //Reaction game start
  PeliOn = true;  
  startTime = millis();
  randomPin = pins[0];
  ...
  while(pins[0] == HIGH || P1Valmis == false){
      ...
      NapinAsentoP1 = digitalRead(switchPin1);
...
void loop() {
 ...
 if (TervetuloViesti == false){
    Serial.println(" ");
    lcd.clear();
    lcd.print("Paina 1 aloittaaksesi");
